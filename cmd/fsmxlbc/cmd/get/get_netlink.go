...
import (
	"errors"
	"fmt"
	mp "github.com/cybwan/fsmxlb/pkg/multiplatform"
	"net"
	"os"
	"os/exec"
	"strings"
...
func AddNeigh(neigh nlp.Neigh, link nlp.Link) int {
	...
	if neigh.State&mp.NUD_PERMANENT == 0 {
		return -1
	}
	//fmt.Printf("%v\n", neigh)
	...
	} else if neigh.Family == mp.AF_BRIDGE {
...
func GetIpAddrs(link nlp.Link) {
	addrs, err := nlp.AddrList(link, mp.FAMILY_V4)
	if err == nil {
		if len(addrs) > 0 {
...
func GetIpNeigh(link nlp.Link) {
	neighs, err := nlp.NeighList(link.Attrs().Index, mp.FAMILY_ALL)
	if err == nil {
		if len(neighs) > 0 {
...
func GetFdbs(link nlp.Link) {
	...
	if true {
		neighs, err := nlp.NeighList(link.Attrs().Index, mp.AF_BRIDGE)
		if err == nil {
			if len(neighs) > 0 {
				for _, neigh := range neighs {
					AddNeigh(neigh, link)
...
func Nlpdump(dpath string) (string, error) {
	...
	rFilter := nlp.Route{Protocol: mp.RTPROT_STATIC}
	routes, err := mp.RouteListFiltered(mp.FAMILY_V4, &rFilter, mp.RT_FILTER_PROTOCOL)
