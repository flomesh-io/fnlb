...
func NewGetSessionULCLCmd(restOptions *xlbcli.RESTOptions) *cobra.Command {
...
func PrintGetSessionULCLResult(resp *http.Response, o xlbcli.RESTOptions) {
	ulclresp := xlbcli.UlclInformationGet{}
	var data [][]string
...
func SessionUlClAPICall(restOptions *xlbcli.RESTOptions) (*http.Response, error) {
	client := xlbcli.NewXlbClient(restOptions)
	ctx := context.TODO()
	var cancel context.CancelFunc
	if restOptions.Timeout > 0 {
...
func SessionUlCldump(restOptions *xlbcli.RESTOptions, path string) (string, error) {
	// File Open
