...
func NewGetEndPointCmd(restOptions *xlbcli.RESTOptions) *cobra.Command {
...
func PrintGetEPResult(resp *http.Response, o xlbcli.RESTOptions) {
	epResp := xlbcli.EPInformationGet{}
	var data [][]string
...
func EPAPICall(restOptions *xlbcli.RESTOptions) (*http.Response, error) {
	client := xlbcli.NewXlbClient(restOptions)
	ctx := context.TODO()
	var cancel context.CancelFunc
	if restOptions.Timeout > 0 {
...
func EPdump(restOptions *xlbcli.RESTOptions, path string) (string, error) {
	epResp := xlbcli.EPInformationGet{}
	// File Open
	...
	epMs := xlbcli.EPConfig{}
	for _, ep := range epResp.EPInfo {
		var epm xlbcli.EndPointMod
