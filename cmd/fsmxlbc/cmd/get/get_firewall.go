...
func NewGetFirewallCmd(restOptions *xlbcli.RESTOptions) *cobra.Command {
...
func PrintGetFWResult(resp *http.Response, o xlbcli.RESTOptions) {
	fwresp := xlbcli.FWInformationGet{}
	var data [][]string
...
func MakeFirewallOptionToString(t xlbcli.FwOptArg) (ret string) {
	if t.Allow {
		ret = "Allow"
	} else if t.Drop {
		ret = "Drop"
...
func FWAPICall(restOptions *xlbcli.RESTOptions) (*http.Response, error) {
	client := xlbcli.NewXlbClient(restOptions)
	ctx := context.TODO()
	var cancel context.CancelFunc
	if restOptions.Timeout > 0 {
...
func FWdump(restOptions *xlbcli.RESTOptions, path string) (string, error) {
	// File Open
