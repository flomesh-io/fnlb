...
func NewGetMirrorCmd(restOptions *xlbcli.RESTOptions) *cobra.Command {
	var GetMirrorCmd = &cobra.Command{
		Use:     "mirror",
		Short:   "Get a Mirror",
		Long:    `It shows Mirror Information in the fsmxlb`,
...
func PrintGetMirrorResult(resp *http.Response, o xlbcli.RESTOptions) {
	Mirrorresp := xlbcli.MirrorGet{}
	var data [][]string
...
func MakeMirrInfoString(infos xlbcli.MirrInfo) (ret string) {
	// Print Mirror Type
	if infos.MirrType == 0 {
		ret = "Type : SPAN\n"
...
func MakeAttachmentToString(objs xlbcli.MirrObjType) (ret string) {
	if objs == 1 {
		ret = "Port"
	} else if objs == 2 {
		ret = "LB Rule"
