...
func NewCreateVxlanPeerCmd(restOptions *xlbcli.RESTOptions) *cobra.Command {
	var createvxlanCmd = &cobra.Command{
		Use:   "vxlanpeer <Vnid> <PeerIP>",
		Short: "Create a vxlan",
		Long: `Create a vxlan using fsmxlb.

ex) fsmxlbc create vxlan-peer 100 30.1.3.1
`,
		...
		Run: func(cmd *cobra.Command, args []string) {
			var vxlanMod xlbcli.VxlanPeerMod
			// Make vxlanMod
...
func ReadCreateVxlanPeerOptions(o *xlbcli.VxlanPeerMod, args []string) error {
	if len(args) > 2 {
...
func VxlanPeerAPICall(restOptions *xlbcli.RESTOptions, vxlanModel xlbcli.VxlanPeerMod, url string) (*http.Response, error) {
