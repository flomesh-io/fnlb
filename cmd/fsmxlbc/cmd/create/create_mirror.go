...
func NewCreateMirrorCmd(restOptions *xlbcli.RESTOptions) *cobra.Command {
	...
	var createmirrorCmd = &cobra.Command{
		...
		Long: `Create a Mirror using fsmxlb
...
ex) fsmxlbc create mirror mirr-1 --mirrorInfo="type:0,port:hs0" --targetObject="attachement:1,mirrObjName:hs1"
...
func ReadCreateMirrorOptions(o *xlbcli.MirrMod, args []string) error {
	if len(args) > 2 {
...
func GetMirrorInfoPairList(o *xlbcli.MirrMod, MirrInfo []string) error {
...
func GetTargetObjPairList(o *xlbcli.MirrMod, TargerObj []string) error {
	for _, mirrorArg := range TargerObj {
		...
		if len(mirrorArgsPair) != 2 {
			...
			o.Target.AttachMent = xlbcli.MirrObjType(attachement)
		}
...
func MirrorAPICall(restOptions *xlbcli.RESTOptions, mirrorModel xlbcli.MirrMod) (*http.Response, error) {
