...
// rule specific fsmxlb constants
const (
...
func RulesInit(zone *Zone) *RuleH {
	...
	if certExists == true && keyExists == true {
		...
		if err != nil {
			tk.LogIt(tk.LogError, "Error loading fsmxlb certificate %s and key file %s",
				certFile, keyFile)
		}
		nRh.tlsCert = cert
	}
...
func (R *RuleH) AddNatLbRule(serv cmn.LbServiceArg, servSecIPs []cmn.LbSecIpArg, servEndPoints []cmn.LbEndPointArg) (int, error) {
	...
	r.secIP = nSecIP
	// Per LB end-point health-check is supposed to be handled at kube-fsmxlb/CCM,
	// but it certain cases like stand-alone mode, fsmxlb can do its own
