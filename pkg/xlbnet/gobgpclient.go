...
func (gbh *GoBgpH) goBgpSpawn() {
	...
	// We need some cool-off period for fsmxlb to self sync-up in the cluster
	time.Sleep(GoBGPInitTiVal * time.Second)
	for {
		cfgOpts := ""

		if _, err := os.Stat("/etc/gobgp/gobgp.conf"); errors.Is(err, os.ErrNotExist) {
			if _, err := os.Stat("/etc/gobgp/gobgp_fsmxlb.yaml"); errors.Is(err, os.ErrNotExist) {
				time.Sleep(2000 * time.Millisecond)
				continue
			}
			cfgOpts = "-t yaml -f /etc/gobgp/gobgp_fsmxlb.yaml"
		} else {
